...
def create_theme(theme: theme_schema.ThemeCreate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    db_theme = models.Theme(**theme.dict(), id=uuid.uuid4(), created_by_user_id=current_user.id)
    db.add(db_theme)
    db.commit()
    db.refresh(db_theme)
    return db_theme
...
def update_theme(theme_id: uuid.UUID, theme: theme_schema.ThemeUpdate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
...
def delete_theme(theme_id: uuid.UUID, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    db_theme = db.query(models.Theme).filter(models.Theme.id == theme_id).first()
    if db_theme is None:
        raise HTTPException(status_code=404, detail="Theme not found")
    if db_theme.created_by_user_id != current_user.id:
...
def set_theme_preference(theme_id: uuid.UUID, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
