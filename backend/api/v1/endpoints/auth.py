...
@router.post("/facebook/ios-login", response_model=Token)
async def facebook_ios_login(request: Request):
    ...
    try:
        user_info = await facebook_get_user_info(access_token)
        if not user_info:
            raise HTTPException(status_code=401, detail="Invalid Facebook token")

        email = user_info.get("email")
        if not email:
            raise HTTPException(status_code=400, detail="Facebook account does not have an email.")
        ...
        if existing_user:
            ...
            await database.execute(users.insert().values(
                id=user_id,
                family_id=family_id,
                first_name=user_info.get("first_name", "Facebook"),
                last_name=last_name,
                email=email,
                password_hash="",          # unused
                subscription_type="Free"
        ...
        jwt_token = create_access_token(data={"sub": uuid_to_string(user_id),
                                                 "family_id": uuid_to_string(family_id)})
